...
//logic [31:0]                nb_data;
  ...
  function automatic void decode_r_aes_insn(input string mnemonic);
    data_accessed = RS1 | RS2 | RD;
    decoded_str = $sformatf("%s\tx%0d,x%0d,x%0d, %0d", mnemonic, rvfi_rd_addr, rvfi_rs1_addr,
        rvfi_rs2_addr, rvfi_insn[31:30]);
  endfunction

  function automatic void decode_r_sha_insn(input string mnemonic);
    data_accessed = RS1 | RD;
    decoded_str = $sformatf("%s\tx%0d,x%0d", mnemonic, rvfi_rd_addr, rvfi_rs1_addr);
  endfunction
  ...
  always @(posedge clk_i) begin
    ...
    //RVFI Trace
      ...
      rd_float = 1;
        ...
        // four aes insturctions
        AES32ESMI : decode_r_aes_insn("aes32esmi");
        AES32ESI  : decode_r_aes_insn("aes32esi");
        AES32DSI  : decode_r_aes_insn("aes32dsi");
        AES32DSMI : decode_r_aes_insn("aes32dsmi");

        SHA256SIG0 : decode_r_sha_insn("sha256sig0");
        SHA256SIG1 : decode_r_sha_insn("sha256sig1");
        SHA256SUM0 : decode_r_sha_insn("sha256sum0");
        SHA256SUM1 : decode_r_sha_insn("sha256sum1");
